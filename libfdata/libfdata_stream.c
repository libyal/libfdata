     ...
     libcerror_error_t **error )
{
	...
	else
	{
		...
		internal_stream->size -= previous_segment_size;
	}
	...
	/* Make sure the stream has a mapped range entry for every segment
	 */
	if( libcdata_array_get_entry_by_index(
	     internal_stream->mapped_ranges_array,
	     segment_index,
         ...
         libcerror_error_t **error )
{
	...
	while( buffer_size > 0 )
	{
		...
		if( (size64_t) internal_stream->segment_data_offset >= segment_data_size )
		{
			if( (size64_t) internal_stream->segment_data_offset > segment_data_size )
			{
				libcerror_error_set(
	...
	while( buffer_size > 0 )
	{
		...
		if( write_count != (ssize_t) write_size )
		{
			libcerror_error_set(
			 ...
			 "%s: unable to write segment: %d data at offset: %" PRIi64 ".",
			 function,
			 internal_stream->current_segment_index,
			 segment_offset + internal_stream->segment_data_offset );
